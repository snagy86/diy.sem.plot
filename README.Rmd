...
Users manually specify node positions using x-y coordinates, as well as where on the perimeter of each node (top, bottom, left, or right) each path should begin and end. From there, the primary function `diyPaths()` renders the diagram, automatically inserting estimates centered on the midpoint and adjusting each node's shape to match its variable type. A range of fine-tuning options are included, facilitating the creation of a path diagram exactly as you envision it, entirely within R.
...
Below is a brief example of what the function is capable of. For more examples plus a full explanation of work flow and available arguments see the package's vignette. 
